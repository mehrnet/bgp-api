...
install_base_tools() {
  ...
  for command in curl jq openssl sha256sum tar flock awk sed grep install; do
    command -v "$command" >/dev/null 2>&1 || missing=true
  done
  [ "$missing" = false ] && return
  manager="$(package_manager)" || die "install curl, jq, openssl, coreutils, tar, and util-linux, then retry"
  step "Installing required command-line tools"
  case "$manager" in
    apt-get)
      apt-get update
      DEBIAN_FRONTEND=noninteractive apt-get install -y ca-certificates curl jq openssl coreutils tar util-linux
      ;;
    dnf) dnf install -y ca-certificates curl jq openssl coreutils tar util-linux ;;
    yum) yum install -y ca-certificates curl jq openssl coreutils tar util-linux ;;
    pacman) pacman -Sy --noconfirm ca-certificates curl jq openssl coreutils tar util-linux ;;
    zypper) zypper --non-interactive install ca-certificates curl jq openssl coreutils tar util-linux ;;
    apk) apk add --no-cache ca-certificates curl jq openssl coreutils tar util-linux ;;
  esac
}
...
0 6 * * * root $INSTALLED_SCRIPT --update >> /var/log/mehrnet-bgp-api-update.log 2>&1
EOF
...
if [ "$MODE" = update ]; then
  ...
  install_docker
  step "Checking for a newer BGP dataset"
...
[ ! -e "$ENV_FILE" ] || die "bgp-api is already installed in $INSTALL_DIR; use $INSTALLED_SCRIPT --update"
...
say "Update: $INSTALLED_SCRIPT --update"
